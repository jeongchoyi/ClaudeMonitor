...
- 말풍선/캐릭터 클릭 → 해당 세션의 iTerm2/Terminal 탭으로 이동
- 드래그로 위치 이동
- Settings UI에서 세션 추가/삭제/순서변경
- `/monitor` 슬래시 커맨드로 세션 자동 등록

## Install

```bash
git clone https://github.com/choyi/ClaudeMonitor.git
cd ClaudeMonitor
...
> 로그인 시 자동 실행하려면 `System Settings > General > Login Items`에 추가
...
| 드래그 | 위치 이동 |
| Ctrl+클릭 / 우클릭 | Settings / Reset Position / Quit |
